...
class Minor_node_centred_Subgraph:
    ...
    def get_fraud_subgraph(self):
        fraud_nodes = np.where(np.array(self.dataframe['Class']) == 1)[0]
...
class Major_node_centred_downsampling:
    ...
    def apply_mlp(self):
        ...
        mlp_model = MLP(input_size=16, output_size=2)  # 2 表示输出两类，keep 和 drop
...
def get_args():
    ...
    parser.add_argument('--gamma','-gamma',type=int,default=0.2)
...
if __name__ == "__main__":
    ...
    g.ndata['labels'] = torch.LongTensor(np.array(df['Class']))


    # 使用例子
    # graph 和 df 是你的图和数据框
    dgl.save_graphs("ccfraud_graph1.bin", [g])
    print(g)
